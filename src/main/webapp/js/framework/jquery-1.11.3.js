		...
		(
				...
				function(window, noGlobal) {
					...
					function isArraylike(obj) {
						...
						var length = "length" in obj && obj.length, type = jQuery
								.type(obj);
					...
					(function(window) {
						...
						setDocument = Sizzle.setDocument = function(node) {
							var hasCompare, parent, doc = node
								...
								"CHILD" : function(type, what, argument, first,
									...
									return first === 1 && last === 0
											...
											function(elem, context, xml) {
												var cache, outerCache, node, diff, nodeIndex, start, dir = simple !== forward
														? "nextSibling"
							...
							pseudos : {
								...
								"not" : markFunction(function(selector) {
									...
									return matcher[expando]
											? markFunction(function(seed,
													matches, context, xml) {
												var elem, unmatched = matcher(
														seed, null, xml, []), i = seed.length;
						...
						function matcherFromTokens(tokens) {
							var checkContext, matcher, j, len = tokens.length, leadingRelative = Expr.relative[tokens[0].type], implicitRelative = leadingRelative
									|| Expr.relative[" "], i = leadingRelative
									? 1
									: 0,
					...
					jQuery
							.extend({
								...
								when : function(subordinate /* , ..., subordinateN */) {
									...
									remaining = length !== 1
											|| (subordinate && jQuery
													.isFunction(subordinate.promise))
											? length
											: 0,
					...
					function internalRemoveData(elem, name, pvt) {
						...
						cache = isNode ? jQuery.cache : elem, id = isNode
								? elem[jQuery.expando]
								: jQuery.expando;
					...
					jQuery.fn.extend({
						...
						triggerHandler : function(type, data) {
							var elem = this[0];
							if (elem) {
								return jQuery.event.trigger(type, data, elem,
										true);
							}
						}
					});
					...
					function getAll(context, tag) {
						var elems, elem, i = 0, found = typeof context.getElementsByTagName !== strundefined
					...
					function manipulationTarget(elem, content) {
						return jQuery.nodeName(elem, "table")
								&& jQuery.nodeName(content.nodeType !== 11
										? content
									...
									return this
											.each(function() {
												var index, data = jQuery
														...
														+ "queueHooks"], timers = jQuery.timers, length = queue
														? queue.length
														: 0;
					...
					jQuery.each(["toggle", "show", "hide"], function(i, name) {
						...
						jQuery.fn[name] = function(speed, easing, callback) {
							return speed == null || typeof speed === "boolean"
									...
									: this.animate(genFx(name, true), speed,
											easing, callback);
						};
					});
					...
					jQuery.each(jQuery.expr.match.bool.source.match(/\w+/g),
							function(i, name) {
								...
								attrHandle[name] = getSetInput
									...
									if (!isXML) {
										return elem[jQuery.camelCase("default-"
												+ name)]
												? name.toLowerCase()
												: null;
									}
								...
								ajax : function(url, options) {
									...
									if (s.crossDomain == null) {
										...
										s.crossDomain = !!(parts && (parts[1] !== ajaxLocParts[1]
												|| parts[2] !== ajaxLocParts[2] || (parts[3] || (parts[1] === "http:"
												? "80"
